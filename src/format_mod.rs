...
use super::module::{
    Column, Effect, LoopType, Module, ModuleInterface, Note, Pattern, PlaybackMode, Row, MODOptions, Sample, VolEffect
};
use byteorder::{BigEndian, LittleEndian, ReadBytesExt};
use std::{io::{self, SeekFrom}, ops::Index};
...
impl MODModule {
    ...
    fn figure_out_channel_count(&self) -> Option<usize> {
        ...
        match &self._mk {
            ...
            id if id.starts_with(b"TDZ") => { // TakeTracker
                char::from_u32(id[3].into()).unwrap().to_digit(10).map(|n| n as usize)
            }
            id if id.ends_with(b"CHN") => { // Various
                char::from_u32(id[0].into()).unwrap().to_digit(10).map(|n| n as usize)
            }
            id if id.ends_with(b"CH") || id.ends_with(b"CN") => { // FastTracker and TakeTracker resp.
    ...
    fn figure_out_tracker(&self) -> Option<MODTracker> {
        ...
        if channel_count == 0 {return None}
        ...
        for sample in &self.samples {
            if sample.volume > 64 {return None;}
            if sample.finetune > 15 {return None;} else
            if sample.finetune != 0 {finetune_used += 1;}
            ...
            if sample.loop_start == 0 {
                if sample.length != 0 {
                    ...
                    if sample.loop_length == 2 {has_slen_sreplen_one+=1;}
                    else if sample.loop_length == 0 {has_slen_sreplen_zero+=1;}
                } else {
                    if sample.loop_length != 0 {no_slen_sreplen_one+=1;}
                    else {no_slen_sreplen_zero+=1;}
                    if sample.volume != 0 {no_slen_has_volume+=1;}
                }
            }
        }
        ...
        for j in 0x11..0x1F {
            if pfx[j] != 0 && finetune_used != 0 {
                if self.restart_point != 0x7F && self.restart_point != 0x78 {
                    ...
                    if amiga_range_notes {return Some(MODTracker::Protracker)}
                    else {return Some(MODTracker::ScreamTracker)}
                }
            }
        }
        ...
        if pfx[0x5] != 0 || pfx[0x6] != 0 ||
           pfx[0x7] != 0 || pfx[0x9] != 0 {
            return Some(MODTracker::ProtrackerCompatible)
        }

        if self.restart_point != 0 && self.restart_point <= self.order_list_length &&
           has_slen_sreplen_zero <= has_slen_sreplen_one &&
           no_slen_sreplen_zero <= no_slen_sreplen_one &&
        ...
        if channel_count != 4 {Some(MODTracker::GenericMultichannel)} else {Some(MODTracker::ProtrackerCompatible)}
    }
...
impl ModuleInterface for MODModule {
        ...
        self.samples
            .iter()
            .map(|s| Sample {
                base_frequency: ((if self.is_amiga() {8287.0} else {8363.0}) * (s.finetune as f64 / 96.0).exp2()).round() as u32,
                loop_type: if s.loop_length > 2 { LoopType::Forward } else { LoopType::None },
            ...
            for r in p {
                ...
                for (i, c) in r.iter().enumerate() {
                    let oc = Column {
                        ...
                        vol: if c.effect == 0xc {VolEffect::Volume(c.effect_value)} else {VolEffect::None},
                        instrument: c.instrument,
                        effect: match c.effect {
                            0x0 => if c.effect_value != 0 {Effect::Arpeggio(c.effect_value)} else {Effect::None(0)},
                            0x1 => if c.effect_value != 0 {Effect::PortaUp(std::cmp::min(c.effect_value,0xDF))} else {Effect::None(0)},
                            0x2 => if c.effect_value != 0 {Effect::PortaDown(std::cmp::min(c.effect_value,0xDF))} else {Effect::None(0)},
                            0x3 => Effect::TonePorta(c.effect_value),
                            0x4 => Effect::Vibrato(c.effect_value),
                            0x5 => if c.effect_value != 0 {Effect::VolSlideTonePorta(c.effect_value)} else {Effect::None(0)},
                            0x6 => if c.effect_value != 0 {Effect::VolSlideVibrato(c.effect_value)} else {Effect::None(0)},
                            0x7 => Effect::Tremolo(c.effect_value),
                            0x8 => if !self.is_amiga() {Effect::FineSetPan(c.effect_value)} else {Effect::None(0)},
                            0x9 => Effect::SampleOffset(c.effect_value),
                            0xA => if c.effect_value != 0 {Effect::VolSlide(c.effect_value)} else {Effect::None(0)},
                            ...
                            0xE => match c.effect_value & 0xF0 {
                                ...
                                0xA0 => if c.effect_value != 0 {Effect::VolSlide((c.effect_value & 0x0F) << 4 | 0x0F)} else {Effect::None(0)},
                                0xB0 => if c.effect_value != 0 {Effect::VolSlide((c.effect_value & 0x0F) | 0xF0)} else {Effect::None(0)},
                            ...
                            0xF => if c.effect_value <= 0x1F {Effect::SetSpeed(c.effect_value)} else {Effect::SetTempo(c.effect_value)}
                            _ => Effect::None(c.effect_value),
                        },
                    };
    ...
    fn module(&self) -> Module {
        Module {
            mode: PlaybackMode::MOD(MODOptions { amiga: self.is_amiga() }),
